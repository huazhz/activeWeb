...
class ArticleController extends AdminController {
    ...
    protected function getMenu($ismenu = null){
        ...
        $data = M('auth_group')->query("select a.rules from yershop_auth_group_access as b,yershop_auth_group as a where a.id = b.group_id and b.uid = $uid");
        $rules = $data[0]['rules'];
        $arr = array();
        $arr = explode(',',$rules);
        $bool = in_array(74,$arr);
        //没有权限的分类则不显示
        if(!IS_ROOT && !$bool){
    ...
    public function update(){
		...
		$category_id = M('category');
	 	$pid['pid'] = $_POST['category_id'];
// 		$aa  = $category_id->field('id')->where($pid)->select();
// 	    $_POST['category_id'] = $aa[0]['id'];
    	$data = array('title'=>$_POST['title'],'uid'=>session("user_auth.uid"),'description'=>$_POST['description'],'status'=>1,'name'=>$_POST['name'],'type'=>$_POST['type'],'display'=>$_POST['display'],'level'=>$_POST['level'],'cover_id'=>$_POST['cover_id'],'view'=>$_POST['view'],'comment'=>$POST['comment'],'create_time'=>$_POST['create_time'],'deadline'=>$_POST['deadline'],'update_time'=>$_POST['update_time'],'pid'=>$_POST['pid'],'model_id'=>$_POST['model_id'],'category_id'=>$_POST['category_id']);
    	//存入document_article表的数据
    	//var_dump($data);
    	$article = array('parse'=>$_POST['parse'],'content'=>$_POST['content']);
    	//实例化两model类
    	$document = M('document');
    	$m = M();
    	$darticle = M('document_article');
		//获取id值，如果有值说明已存入草稿，进行更新操作，否则插入数据
    	$id = $_POST['id'];
    
    	if($id){
    		$where1['id'] = $id;
    		$res1 = $document->where($where1)->save($data);

    		$res2 = $darticle->where($where1)->save($article);
    		...
    		$m->startTrans();
    		$res1 = $document->add($data);
    		$article['id'] = $res1;		
    		$res2 = $darticle->where($where2)->add($article);
    		//var_dump($article);
    		if($res1 && $res2){
    			$m->commit();
    ...
    public function clear(){
      ...
      //  $res1 = D('Document_article')->remove();
        if($res !== false){
