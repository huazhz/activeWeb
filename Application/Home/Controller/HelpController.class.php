...
class HelpController extends HomeController {
	...
	public function lists(){
		...
		;//获取最大的文章id
		$list=M("document")->where("category_id=$pid and status=1")->select();
	...
	public function detail(){
	    ...
	    if(!empty($id)){
		    ...
		    //$info = D('Document')->find($id);
		   //echo  D('Document')->getLastSql();
		   $data = D('Document')->query("select a.title,a.create_time,a.category_id,b.content from yershop_document as a,yershop_document_article as b where a.id=b.id and a.id=$id");
			$info = $data[0];
		  // echo D('Document')->getLastSql();
		   // var_dump($info);
		}
