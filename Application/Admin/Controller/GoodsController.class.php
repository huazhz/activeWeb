...
class GoodsController extends AdminController {
    ...
    public function edit(){
     ...
     // echo $Document->getLastSql();
        ...
        $data1 = $product->where("id = $id")->find();
		//var_dump($data1);die;
		//$data = array('content'=>$data1['content'],'area'=>$data1['area']);
		$data['content'] = $data1['content'];
		$data['area'] = $data1['area'];
        $picture = D('picture');
        $cover_id = $data['cover_id'];
        $data2 = $picture->where("id= $cover_id")->find();
	...
	//var_dump(session("user_auth.uid"));die;
     ...
     $_POST['uid'] = session("user_auth.uid");
   	$_POST['baoming_info'] = implode (',',$_POST['baoming_info']);
   	$data1 = array('content'=>$_POST['content'],'place'=>$_POST['place'],'area'=>$_POST['area']);
   	$data2 = array('title'=>$_POST['title'],'baoming_info'=>$_POST['baoming_info'] ,'deadline'=>$_POST['deadline'],'model_id'=>$_POST['model_id'],'uid'=>$_POST['uid'],'status'=>$_POST['status'],'category_id'=>$_POST['category_id'],'cover_id'=>$_POST['cover_id'],'create_time'=>$_POST['create_time'],'starttime'=>$_POST['starttime'],'baoming'=>$_POST['baoming'],'if_pay'=>$_POST['if_pay'],'price'=>$_POST['price'],'update_time'=>time(),'place'=>$_POST['place']);
        $document   =   D('Document');
       ...
       if($_POST['id']){
       	...
       	$res1 = $document->where("id=".$_POST['id'])->save($data2);
       	$res2 = $product->where("id=".$_POST['id'])->save($data1);
       }else{
       	$data2['create_time'] = time();
       	$res1 = $document->add($data2);
       	$data1['id'] = $res1;
       	$res2 = $product->add($data1);
       }
