...
LEFT JOIN `yershop_document_product` AS c ON a.id=c.id
   ...
   /*活动详情*/
		$xiangqing=M('document')->query("
				SELECT a.id,b.id,b.content FROM `yershop_document` AS a LEFT JOIN `yershop_document_product` AS b ON
		...
		$member=M('member')->query('select nickname from yershop_member');	
...
//	$category = $this->category($info['category_id']);
   ...
   //最近浏览
	...
	public function submint(){
		...
		$apply=$show->add($post);
		if(!empty($apply)){
			$this->success('报名成功',U('__CONTROLLER__/index'));
